...
export const formatValueWithIndianNumericPrefix = (
	...
	if (integerPart.length <= 3) {
		return `${prefixVal}${integerPart}${decimalPart ? "." + decimalPart : ""}`;
	}

	// For numbers between 1,000 and 99,999 (Thousands and Ten Thousands)
	if (integerPart.length <= 5) {
		return `${prefixVal}${integerPart.slice(0, -3)},${integerPart.slice(-3)}${
	...
	return `${prefixVal}${formattedNumber}${decimalPart ? "." + decimalPart : ""}`;
};
